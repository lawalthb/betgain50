...
<div id="referral_modal" class="modal">
  ...
  <!-- modal -->
  <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'" id="referralModal">
    <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
      <div x-show="open" x-transition x-transition.duration.300 class="panel border-0 py-1 px-4 rounded-lg overflow-hidden w-full max-w-sm my-8">
        <div class="flex items-center justify-between p-5 font-semibold text-lg dark:text-white" id="referral_title">Register by referral
          <span class="flex-right cursor-pointer hover:text-danger" id="close_referral">X</span>
        </div>
        <div class="p-5 mb-4">
          ...
          <form id="signup_form_ref">
            ...
            <div class="relative mb-4">
              ...
              $ref_co = $_GET['referral'];
              }else{
              $ref_co = '';
              }
              @endphp
              <input type="text" value="{{$ref_co}}" name="referral_code" id="referral_code" required class="form-input ltr:pl-10 rtl:pr-10" />
            </div>


            <button type="submit" id="submit_register2" class="btn btn-primary w-full">Submit</button>
          ...
          <div class="p-5 border-t border-[#ebe9f1] dark:border-white/10 hover:text-white">
            ...
            <!-- <p class="text-center text-white-dark dark:text-white-dark/70">Looking to <a href="javascript:;" id="referralLink" class="text-[#515365] hover:underline dark:text-white-dark">create an account?</a></p> -->
            </footer>
          </div>
        </div>
      </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      $("#close_referral").click(function(event) {
        $("#referral_modal").css("display", "none");

      });

    });
    document.getElementById("signup_form_ref").addEventListener("submit", async function(event) {
      event.preventDefault();
      const submit_register2 = document.getElementById('submit_register2');
      submit_register2.setAttribute('disabled', '');
      submit_register2.innerText = 'Please Waiting...';
      ...
      try {
        ...
        if (response.ok) {
          ...
          const removeRegisterModal = document.getElementById('referral_modal');
